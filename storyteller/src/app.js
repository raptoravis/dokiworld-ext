...
function startConfiguredExperience() {
  ...
  if (!root || pathNeedsLlm(root.id)) {
    showWaiting();
    post("dokiworld-world-episode-start");
    return;
  }
  ...
  groups.forEach((entry, groupIndex) => {
    ...
    if (
      ...
      regenerate.addEventListener("click", () => {
        ...
        showChatWaiting();
        post("dokiworld-world-chat-regenerate", { playerPersona });
      });
...
function submitReply(value) {
  ...
  elements.lines.append(group);
  showChatWaiting();
  post("dokiworld-world-episode-reply", { playerInput, playerPersona });
}
...
function renderChoices(item) {
  ...
  options.forEach((option, index) => {
    ...
    button.addEventListener("click", () => {
      ...
      showWaiting();
      post("dokiworld-world-episode-choice", {
...
async function openConfiguredApp(config) {
  ...
  try {
    ...
    const runId = `${host.runId}:${Date.now().toString(36)}`;
    activeApp = { app, config, runId, initialized: false };
...
function initializeActiveGame() {
  if (!activeApp || activeApp.initialized) return;
  const { context, grantedScopes } = createGameContext();
  activeApp.initialized = true;
  const appOrigin = new URL(activeApp.app.entryUrl, window.location.origin).origin;
  elements.appFrame.contentWindow?.postMessage({
    type: "dokiworld-game-init",
    protocolVersion: 1,
    gameId: activeApp.app.id,
    runId: activeApp.runId,
    locale,
    grantedScopes,
    context,
    options: {
      configId: activeApp.config.configId,
      rows: activeApp.config.rows,
      columns: activeApp.config.columns ?? activeApp.config.cols,
      moves: activeApp.config.moves,
      timeLimit: activeApp.config.timeLimit,
      targetScore: activeApp.config.targetScore ?? activeApp.config.target,
      seed: activeApp.config.seed,
    },
  }, appOrigin);
}
...
function requestAction(item) {
  ...
  showWaiting();
  post("dokiworld-world-episode-action", { beatId });
}
...
function restartEpisode() {
  ...
  showWaiting();
  post("dokiworld-world-episode-restart");
  window.setTimeout(() => post("dokiworld-world-episode-start"), 0);
}
...
function initialize(message) {
  ...
  post("dokiworld-world-initialized");
...
elements.suggest.addEventListener("click", () => {
  ...
  elements.chatStatus.textContent = copy.thinking;
  post("dokiworld-world-chat-suggest", { playerPersona });
});
...
function requestGeneratedMedia(mediaType) {
  ...
  post("dokiworld-world-chat-generate-media", { mediaType, playerPersona });
}
...
window.addEventListener("message", (event) => {
  if (event.source === window.parent) {
    ...
    if (
      message.type === "dokiworld-world-init"
    ...
    if (message.type === "dokiworld-world-episode") acceptEpisode(message.utterances);
    if (message.type === "dokiworld-world-chat-regenerated") {
      elements.lines.querySelector(".message-group.is-ai:last-of-type")?.remove();
      acceptEpisode(message.utterances);
    }
    if (message.type === "dokiworld-world-chat-media") {
      elements.chatStatus.textContent = "";
    ...
    if (message.type === "dokiworld-world-chat-media-error") {
      elements.chatStatus.textContent = typeof message.error === "string" ? message.error : copy.tryAgain;
      elements.generateImage.disabled = false;
      elements.generateVideo.disabled = false;
    }
    if (message.type === "dokiworld-world-chat-suggestions") {
      elements.chatStatus.textContent = "";
      elements.suggestionPanel.replaceChildren();
    ...
    if (message.type === "dokiworld-world-episode-game" && pendingAction) {
      void openConfiguredApp(message.gameConfig);
    }
    if (message.type === "dokiworld-world-episode-fixed-game-result" && isRecord(message.result)) {
      completeHostedConfiguredApp(message.result);
    }
    if (
      message.type === "dokiworld-world-episode-game-resolved"
      && isRecord(message.result)
      && Array.isArray(message.utterances)
    ) {
      completeHostedConfiguredApp(message.result, message.utterances);
    }
    if (message.type === "dokiworld-world-episode-error") showError();
    return;
  }

  if (!activeApp || event.source !== elements.appFrame.contentWindow || !isRecord(event.data)) return;
  const message = event.data;
  if (message.type === "dokiworld-game-ready") {
    activeApp.initialized = false;
    initializeActiveGame();
    return;
  }
  if (
    message.protocolVersion !== 1
    || message.gameId !== activeApp.app.id
    || message.runId !== activeApp.runId
  ) return;
  ...
  if (message.type === "dokiworld-game-result" && isRecord(message.result)) {
    if (localActionBeat) {
      completeLocalConfiguredApp(message.result);
      return;
    }
    post("dokiworld-world-episode-game-result", {
      configId: activeApp.config.configId,
